...
# RUN TESTS SINGLE_THREADED SO WE CAN FIGURE OUT WHERE'S THE WEIRDNESS (the hang that comes from an async op running without a reap/reclaim or multiple async ops being sent off without inteveneing reclaims/reaps)
        ...
        while True:
            ...
            if (message_type == "R"): # relinquish some cores
                ...
                if (self.current_cores < 0):
                    print("cores went below zero (an async operation was spontaneously generated by zcashd)")
                    self.current_cores = 0
